...
export class CreateClientDto {
  @ApiProperty({ example: 'Acme Corporation', description: 'Business or client name' })
  @Transform(({ value }) => (typeof value === 'string' ? value.trim() : value))
  @IsString()
  @IsNotEmpty({ message: 'Business name is required' })
  businessName: string;

  @ApiPropertyOptional({ example: 'acme-corporation', description: 'Unique URL slug' })
  @Transform(({ value }) => (typeof value === 'string' ? value.toLowerCase().trim() : value))
  @IsString()
  @IsOptional()
  slug?: string;

  @ApiPropertyOptional({ example: 'Leading provider of smart web tech', description: 'Client business description' })
  @Transform(({ value }) => (typeof value === 'string' ? value.trim() : value))
  @IsString()
  @IsOptional()
  description?: string;

  @ApiPropertyOptional({ example: 'contact@acme.test', description: 'Primary contact email' })
  @Transform(({ value }) => (typeof value === 'string' ? value.toLowerCase().trim() : value))
  ...
  @ApiPropertyOptional({ enum: ClientStatus, default: ClientStatus.ACTIVE, description: 'Client status (ACTIVE, INACTIVE, ARCHIVED)' })
