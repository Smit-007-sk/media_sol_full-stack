...
export default function ApexHero() {
  ...
  return (
    <section className="relative min-h-[85vh] bg-[#0B1320] text-white overflow-hidden border-b border-amber-900/30 flex flex-col justify-between" style={{ backgroundColor: 'var(--theme-background, #0B1320)', color: 'var(--theme-text, #ffffff)' }}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-16 relative z-10 text-center w-full">
        <div className="max-w-3xl mx-auto space-y-4">
          <span className="text-xs font-bold uppercase tracking-widest text-[#F59E0B] bg-amber-950/80 px-4 py-1.5 rounded-full border border-amber-800/60 inline-block" style={{ color: 'var(--theme-secondary, #F59E0B)', borderColor: 'var(--theme-secondary, rgba(245,158,11,0.6))' }}>
            FULL-BLEED PRIVATE LUXURY COLLECTION
          </span>

          <h1 className="font-serif text-4xl sm:text-6xl font-bold text-white leading-tight" style={{ fontFamily: 'var(--theme-heading-font, inherit)' }}>
            Architectural Masterpieces <br />
            <span className="italic text-[#F59E0B]" style={{ color: 'var(--theme-primary, #F59E0B)' }}>in Prime Destinations</span>
          </h1>
        </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12 w-full pt-10">
        <div className="bg-[#1E293B]/95 backdrop-blur-xl border border-slate-700/60 rounded-3xl p-6 shadow-2xl flex flex-col md:flex-row items-center justify-between gap-6 text-left">
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-2 text-xs font-bold text-amber-500 uppercase tracking-wider" style={{ color: 'var(--theme-secondary, #f59e0b)' }}>
              <MapPin className="w-3.5 h-3.5" />
              <span>{active.location}</span>
            </div>
            <h3 className="font-serif text-2xl font-bold text-white">{active.title}</h3>
            <div className="text-xl font-bold text-[#F59E0B]" style={{ color: 'var(--theme-primary, #F59E0B)' }}>{active.price}</div>
          </div>

          {/* Specs Pill List */}
          <div className="flex items-center gap-4 text-xs font-semibold text-slate-300">
            <div className="flex items-center gap-1.5 bg-[#0F172A] px-3.5 py-2 rounded-xl border border-slate-800">
              <Bed className="w-4 h-4 text-amber-500" style={{ color: 'var(--theme-secondary, #f59e0b)' }} />
              <span>{active.beds} Beds</span>
            </div>
            <div className="flex items-center gap-1.5 bg-[#0F172A] px-3.5 py-2 rounded-xl border border-slate-800">
              <Bath className="w-4 h-4 text-amber-500" style={{ color: 'var(--theme-secondary, #f59e0b)' }} />
              <span>{active.baths} Baths</span>
            </div>
            <div className="flex items-center gap-1.5 bg-[#0F172A] px-3.5 py-2 rounded-xl border border-slate-800">
              <Maximize2 className="w-4 h-4 text-amber-500" style={{ color: 'var(--theme-secondary, #f59e0b)' }} />
